...
import { ArrowLeft, Loader2, Send, Save, ImageIcon, User, Upload, Video, X } from 'lucide-react';
...
export default function AdminRespondPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!questionData) { router.push('/admin/queue'); return; }
        setQuestion(questionData);

        const [aiRes, claimRes] = await Promise.all([
        ...
        // Admins can always view any student's profile
    ...
    try {
      ...
      if (videoFile) {
        ...
        if (uploadedPath) {
          ...
          // Not persisting a signed URL — it expires. Playback resolves a
          // fresh one from video_storage_path on demand (see
          // lib/supabase/video-url.ts). See the identical fix in
          // creator/respond/[id]/page.tsx for the full explanation.
          finalVideoUrl = null;
        }
      }

      // Admins can respond even without a claim — create a claim if needed
      let claimId = claim?.id;
      if (!claimId) {
        const { data: newClaim, error: claimErr } = await supabase
          .from('question_claims')
          .insert({ question_id: questionId, creator_id: user.id, status: 'in_progress' })
          .select('id')
          .single();
      ...
      await Promise.all([
        supabase.from('question_claims').update({ status: 'completed', completed_at: new Date().toISOString() }).eq('id', claimId),
  ...
  if (!question) return (
    <div className="text-center py-12">
      ...
      <Button className="mt-4" asChild><Link href="/admin/queue">Back to Queue</Link></Button>
    </div>
  );
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild>
          <Link href="/admin/queue"><ArrowLeft className="h-5 w-5" /></Link>
        </Button>
        <div>
          <h1 className="text-2xl font-bold">Create Response</h1>
          <p className="text-muted-foreground">Provide a detailed explanation for the student</p>
        </div>
      </div>
                    ...
                    {attachments.map((att) => (
                      att.signedUrl && att.mime_type?.startsWith('image/') ? (
                        <div key={att.id} className="relative rounded-lg overflow-hidden border bg-muted">
                          <img src={att.signedUrl} alt={att.file_name} className="w-full object-contain max-h-72" />
                          <p className="text-xs text-muted-foreground px-2 py-1 bg-background/80">{att.file_name}</p>
                        </div>
                      ) : att.signedUrl ? (
                        <a key={att.id} href={att.signedUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1">
          ...
          {studentProfile && (
            <Card className="border-dashed">
              ...
              <CardContent className="space-y-2 text-sm">
                {studentProfile.full_name && <p className="font-medium">{studentProfile.full_name}</p>}
                <div className="flex flex-wrap gap-3 text-muted-foreground">
                  {studentProfile.grade_level && <span>Grade {studentProfile.grade_level}</span>}
                  {studentProfile.target_sat_score && <span>Target SAT: {studentProfile.target_sat_score}</span>}
                </div>
                ...
                {studentProfile.bio && <p className="text-xs text-muted-foreground line-clamp-2">{studentProfile.bio}</p>}
                <Link href={`/admin/students/${question.user_id}`} className="text-xs text-primary hover:underline">View full profile</Link>
              </CardContent>
            </Card>
          )}
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <Textarea id="explanation" placeholder="Explain the concept step by step. Use markdown for formatting..." value={explanation} onChange={(e) => setExplanation(e.target.value)} rows={10} className="font-mono" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="teachingNotes">Teaching Notes (optional)</Label>
              <Textarea id="teachingNotes" placeholder="Additional context or tips for this student..." value={teachingNotes} onChange={(e) => setTeachingNotes(e.target.value)} rows={3} />
            </div>
            ...
            <div className="space-y-3">
              ...
              {videoPreview ? (
                <div className="relative rounded-lg overflow-hidden border bg-muted">
                  ...
                  <Button type="button" variant="destructive" size="sm" className="absolute top-2 right-2" onClick={clearVideo}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              ) : videoUploading ? (
                ...
                <div className="space-y-3">
                  ...
                  <Button type="button" variant="outline" onClick={() => videoInputRef.current?.click()} className="w-full h-24 border-2 border-dashed flex flex-col items-center justify-center gap-2">
                  ...
                  <div className="space-y-1">
                    <Input type="url" placeholder="https://youtube.com/watch?v=..." value={videoUrl} onChange={(e) => { setVideoUrl(e.target.value); clearVideo(); }} disabled={!!videoFile} />
