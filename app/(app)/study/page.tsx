...
export default function StudyPage() {
  ...
  const handleSelectMode = useCallback(async (mode: ModeDefinition) => {
    if (mode.key === 'question_bank') {
      router.push('/question-bank/browse');
      return;
    }
  ...
  const handleStartSession = async () => {
    ...
    // Challenge Mode manages its own session creation (server-side, live
    // adaptive) — it doesn't go through the shared fixed-list engine at all.
    if (selectedMode.key === 'challenge_mode') {
      router.push(section && section !== 'all' ? `/practice/challenge?section=${section}` : '/practice/challenge');
      return;
    }
        ...
        <Card>
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="border-t pt-4 space-y-4">
              ...
              {/* Domain filter when section picked */}
              ...
              {/* Mode-specific notes */}
              {def.key === 'infinite_practice' && (
                <div className="p-3 rounded-lg bg-info/10 border border-info/25 text-sm text-info">
                  Questions keep coming until you choose to stop. Your progress is saved continuously.
                </div>
              )}
              {def.key === 'timed_practice' && (
                <div className="p-3 rounded-lg bg-warning/10 border border-warning/25 text-sm text-warning">
                  <Clock className="inline h-4 w-4 mr-1.5" />
                  {questionCount * 70}s total — about {Math.round(questionCount * 70 / 60)} minutes at 70s per question.
                </div>
              )}
              {def.key === 'exam_simulation' && (
                <div className="p-3 rounded-lg bg-success/10 border border-success/25 text-sm text-success">
                  <GraduationCap className="inline h-4 w-4 mr-1.5" />
                  Full 98-question exam in Bluebook style. No answer reveals until the test is complete.
                </div>
              )}
              {def.key === 'review_mistakes' && (
                <div className="p-3 rounded-lg bg-warning/10 border border-warning/25 text-sm text-warning">
                  Only questions you previously answered incorrectly. You need at least one wrong answer on record.
                </div>
              )}
              {def.key === 'bookmarked_questions' && (
                <div className="p-3 rounded-lg bg-info/10 border border-info/25 text-sm text-info">
                  Only your bookmarked questions. Bookmark questions during any session to add them here.
                </div>
              )}
              {def.key === 'weakest_topics' && (
                <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/25 text-sm text-destructive">
                  <TrendingDown className="inline h-4 w-4 mr-1.5" />
                  Questions from your lowest-accuracy skills based on past sessions. More history = better targeting.
                </div>
              )}
              {def.key === 'adaptive_practice' && (
                <div className="p-3 rounded-lg bg-milo/10 border border-milo/25 text-sm text-milo">
                  Difficulty adjusts every few questions. Start at your chosen section, then the engine reacts to your answers.
                </div>
              )}
            </div>
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-0.5">Pick a mode and start practicing</p>
      </div>
      ...
      <div>
        ...
        <Card
          className="cursor-pointer hover:border-primary/50 transition-all hover:shadow-sm group"
          onClick={() => handleSelectMode(qbMode)}
        >
      ...
      <Dialog open={resumeDialogOpen} onOpenChange={setResumeDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              You have an unfinished {selectedMode?.label} session. What would you like to do?
            </DialogDescription>
          </DialogHeader>
          {activeSession && (
            <div className="py-2">
              <div className="p-3 rounded-lg bg-muted text-sm space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    Question {(activeSession.last_question_index ?? 0) + 1} of {activeSession.generated_question_ids?.length ?? activeSession.total_questions ?? '?'}
                  </span>
                </div>
                <div className="flex justify-between">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={handleReviewPrevious} className="gap-2">
              <CheckCircle2 className="h-4 w-4" />Review Results
            </Button>
